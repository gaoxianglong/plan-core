...
1. **用户管理**：查看用户列表、筛选已注销用户、标记退款
2. **订单管理**：查看订单、手动退款
3. **基础数据统计**：用户/会员/打卡概览
4. **系统配置**：头像库、专注类型、推送时间等

### 1.2 核心场景

#### 场景 1：处理注销用户退款（刚需）
运营人员每周查看"已注销用户列表"，筛选出"有付费记录且需要退款"的用户，批量标记后进行退款操作。

#### 场景 2：监控用户活跃度
运营人员查看"用户概览"，了解当前活跃用户数、注销率，评估产品健康度。
...
| **最后活跃时间** | 日期范围选择器 | - |
| **是否需要退款** | 全部/是/否 | 全部 |
| **搜索** | 用户 ID / 昵称 / 微信 openId | - |

**快捷筛选**：
- "已注销且需要退款"：`账号状态=已注销 AND 是否需要退款=是`（一键应用）
...
| 账号状态 | 正常/已注销（标红） | - |
| 注销原因 | "连续 7 天未打卡" / "手动注销"（预留） | - |
| 注销时间 | 账号进入 Deactivated 的时间 | ✓ |
| 会员状态 | 未激活/免费期/会员有效/已到期 | - |
| 会员到期时间 | expireAt | ✓ |
| 连续打卡天数 | 当前连续天数（注销用户显示 0） | ✓ |
| 最后打卡时间 | 最近一次打卡日期 | ✓ |
| 任务总数 | 创建的任务总数（含已删除） | ✓ |
| 完成任务数 | 已完成任务数 | ✓ |
| 专注总时长 | 累计专注时长（小时） | ✓ |
...
**2. 标记退款**
- **触发条件**：账号状态 = 已注销，且有付费订单
- **操作**：弹窗确认，标记"是否需要退款 = 是"
- **标记后**：该用户在列表中高亮显示（背景色浅黄）
- **取消标记**：支持取消标记

**3. 批量标记退款**
- 勾选多个用户，点击"批量标记退款"
- 仅对"账号状态=已注销且有付费订单"的用户生效

**4. 导出 Excel**
- 导出当前筛选结果（最多 10000 条）
- 包含所有表格字段 + 微信 openId + 退款标记时间
...
| 区域 | 内容 |
|------|------|
| 基本信息 | 用户 ID、昵称、头像、注册时间、最后活跃时间、IP 属地 |
| 账号状态 | 正常/已注销、注销时间、注销原因、连续打卡天数、最后打卡时间 |
...
**快捷筛选**：
- "待退款订单"：`订单状态=支付成功 AND 用户标记为需要退款`（关联用户管理）
...
**2. 手动退款**
- **触发条件**：订单状态 = 支付成功，且用户已标记"需要退款"
- **操作**：
  1. 弹窗确认退款金额（默认=订单金额）
  2. 填写退款原因（必填，如"用户注销退款"）
  3. 点击"确认退款"
- **退款后**：
  - 订单状态 → 已退款
  - 用户会员状态 → 已到期
...
| 新增用户数 | 选定时间范围内新注册用户数 | ✓ |
| 注销用户数 | 选定时间范围内进入 Deactivated 状态的用户数 | ✓ |
| 注销率 | 注销用户数 / 总用户数 × 100% | ✓ |
...
- **用户趋势图**：折线图展示"新增用户数/活跃用户数/注销用户数"（最近 30 天）
...
#### 功能
- **未打卡提醒推送时间**：设置发送窗口（默认：20:00–21:00）
  - 开始时间：时分选择器（如 20:00）
  - 结束时间：时分选择器（如 21:00）
- **推送开关**：全局开启/关闭未打卡提醒
...
- **敏感数据脱敏**：用户手机号/微信 openId 部分隐藏（仅超级管理员可见完整）
