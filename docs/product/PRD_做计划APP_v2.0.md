...
| **打卡（Check-in）** | **存在明确"打卡/完成今日计划"按钮，点击才算打卡**（影响坚持天数统计） | `date`（打卡日期）<br>`checkedAt`（打卡时间）<br>`consecutiveDays`（连续天数） |
...
| **会员（Entitlement）** | **免费期**：非会员免费使用 1 个月，从**首次注册**起算<br>**会员有效期**：购买成功后得到到期时间 | `trialStartAt`（免费期起算时间）<br>`expireAt`（会员到期时间）<br>`status`（未激活/免费期/会员有效/已到期） |
| **账号状态** | 未登录/已登录 | `lastCheckInDate`（最近打卡日期） |
...
stateDiagram-v2
  ...
  NotEntitled --> FreeTrial: 首次注册（记录trialStartAt）
...
stateDiagram-v2
  ...
  LoggedOut --> LoggedIn: 邮箱密码登录成功（生成设备token）
