...
7. [共享内核（Shared Kernel）](#8-共享内核shared-kernel)
...
| **打卡上下文（CheckIn Context）** | 每日打卡、连续天数统计 | 打卡记录、坚持统计 |
...
class User {
    ...
    Email email;                // 邮箱（值对象，唯一标识）
    Password password;          // 密码（值对象，加密存储）
...
public class Password {
    private final String hashedValue;  // 加密后的密码
    ...
    private static void validate(String plainText) {
        ...
        if (plainText.length() < MIN_LENGTH) {
            throw new IllegalArgumentException(
                String.format("密码长度不能少于 %d 位", MIN_LENGTH)
            );
        }
        if (plainText.length() > MAX_LENGTH) {
            throw new IllegalArgumentException(
                String.format("密码长度不能超过 %d 位", MAX_LENGTH)
            );
        }
    }
...
#### 3.2.7 NicknameModifyWindow（昵称修改窗口）
