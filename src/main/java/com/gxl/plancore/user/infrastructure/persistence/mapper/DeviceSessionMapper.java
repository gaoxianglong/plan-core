...
public interface DeviceSessionMapper {
    ...
    @Insert("INSERT INTO device_session (session_id, user_id, device_id, device_name, platform, " +
            ...
            "#{lastLoginIp}, #{lastLoginAt}, #{lastActiveAt}, #{status}, #{loggedOutAt}, #{createdAt}, #{updatedAt})")
    int insert(DeviceSessionPO po);
    ...
    @Select("SELECT * FROM device_session WHERE refresh_token = #{refreshToken} AND status = 'ACTIVE'")
    DeviceSessionPO selectByRefreshToken(@Param("refreshToken") String refreshToken);

    @Select("SELECT * FROM device_session WHERE user_id = #{userId} AND device_id = #{deviceId} AND status = 'ACTIVE'")
    DeviceSessionPO selectActiveByUserIdAndDeviceId(@Param("userId") String userId, @Param("deviceId") String deviceId);
    ...
    @Update("UPDATE device_session SET " +
            "access_token = #{accessToken}, refresh_token = #{refreshToken}, " +
            "expires_at = #{expiresAt}, refresh_expires_at = #{refreshExpiresAt}, " +
            "last_active_at = #{lastActiveAt}, updated_at = #{updatedAt} " +
            "WHERE session_id = #{sessionId}")
    int updateTokens(DeviceSessionPO po);
